...
if($action == 'insert'){
	...
	$cmd = 'update';
	$ro1 = CrmReporteOcurrenciaDR::getItemCHD($chdID,1);
	$descartes_residuos = CrmDescartesResiduos::getItemCHD($chdID);
	if($descartes_residuos->plantaID != NULL){
		$planta = CrmPlantaChd::getItem($descartes_residuos->plantaID);
	}
	...
	{ 
		$count = 0;
		$countdet=0;
		$ro = CrmReporteOcurrenciaDR::getList($chdID);
		foreach ($ro as $valor) {
			$count++;
			${'ro'.$count} = new eCrmReporteOcurrenciaDR();
			${'ro'.$count} = $valor;
			$fraccion = CrmInfraccionDR::getList(${'ro'.$count}->reporteOcurrenciaDRID);
			foreach ($fraccion as $detail) {
				$countdet++;
				${'infraccion'.$count.'_'.$countdet} = new eCrmInfraccionDR();
...
<section class="content">
	<form name="form_ro" id="form_ro" > 
		<div class="row">
			<div class="col-md-12"><h2 class="box-title"><i class="fa fa-inbox"></i>&nbsp; Reporte de Ocurrencias</h2></div>
		</div>
		<div class="row">
		...
		<div class="row">
			...
			<div class="row">
				...
				<div class="col-md-2">
					<label></label>
					<div class="btn btn-primary btn-block" name="btnCrear" id="send_ro">Siguiente&nbsp;&nbsp;<i class="fa fa-chevron-right"></i></div>
				</div>
			</div>
		...
		<script type="text/javascript">
			...
			$(function(){
				prepareForm('#form_ro');
				$('#send_ro').click(function(){
					<?php if($oUser->level != 0){ ?>
						bootbox.confirm({
							title: "Produce - Bureau Veritas",
							message: "Estas seguro de guardar los reportes de Ocurrencias?",
							buttons: {
								cancel: {
							...
							callback: function (result) {
								if(result){
									...
									<?php if($cmd!='update'){ ?>
										$.getJSON('<?php echo $URL_ROOT;?>ajax/form_ro_dr.php?cmd=insert&'+fields3, function(data) {
											if(data.retval==1){
										...
										<?php }else{ ?>
											$.getJSON('<?php echo $URL_ROOT;?>ajax/form_ro_dr.php?cmd=update&ID=<?php echo $chdID;?>&'+fields3, function(data) {
												if(data.retval==1){
													alertify.success(data.message);
													$('.sombra').show();
													setTimeout(function(){
			...
			$('#back_ro').click(function(){
				location.href='<?php echo $URL_ROOT;?>chd/datos_secundarios_dr.html?action=update&ID=<?php echo $chdID; ?>';
			});
		</script>
